...
acc = 0,carga = 0;
      	 	...
      	 	setTimeout(function(){
      	 		$.getJSON('libs/acc_producto',{opc:'cargardat',nombre:prod.val()}).done(function(data){
      	 			...
      	 			$.each(data.info,function(i,dat){
      	 				$('input').not('[name="descr"]').removeAttr('disabled');
	...
	$('#producto').on('submit',function(e){
	  ...
	  //       if(acc==2)
	        ...
	        $('[name="descr"]').removeAttr('disabled');
		...
		else
		{
			$('#fondo').remove();
			$('body').append("<div class='fondo' id='fondo' style='display:none;'></div>");
			$('#fondo').append("<div class='rp' style='display: none; text-align: center' id='rp'><span>Error! - El nombre del producto no se puede cambiar</span></div>");
			setTimeout(function() {
	        	$('#fondo').fadeIn('fast',function(){
	            $('#rp').animate({'top':'350px'},50).fadeIn();
	         	});
